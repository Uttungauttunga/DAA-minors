...
{
return  max(findHeight_T(pt->left),findHeight_T(pt->right))+1;
}
}

int findHeight(TREE *pt)
{
    return findHeight_T(pt->root);
}


int count=0;

int leafcount_T(NODE *pt)
{
    if(pt!=NULL)
    {
